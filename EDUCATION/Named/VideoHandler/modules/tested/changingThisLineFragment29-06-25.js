const changingThisLineFragment = (line, separator, fragmentNumberInОrder, changedСontent) => {
    let anArrayFromTheCurrentRow = line.split(separator);//alert(anArrayFromTheCurrentRow + " В первой строке тела функции строка (line) преобразуется в массив разделяя строку по разделютелю (separator) и полученный массив присваивается переменной anArrayFromTheCurrentRow Дополнение: будет меняться фрагмент под номером - " + fragmentNumberInОrder + " на контент: " + changedСontent);
    ...
    let changingTheArray = pull; //alert(changingTheArray + " Во второй строке, элементу, выбранному по номеру порядка его расположения в массиве (имеет тот же номер что и фрагмент строки разделенный по разделютелю) присваевается новое значение (новый контент из changedСontent), после чего  массив с измененым элементом присваивается переменной changingTheArray.")
    let modifiedString = changingTheArray.toString(); // alert(modifiedString + " В третье строке изменный массив  (changingTheArray), преобразуется в измененую строку, которая присваивается переменной modifiedString.")
